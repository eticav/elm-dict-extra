...
-}
foldlIf : (comparable->Order)->(comparable -> v -> b -> b) -> b -> Dict comparable v -> b
foldlIf comp f acc dict =
  case dict of
    RBEmpty_elm_builtin _ ->
      acc
    RBNode_elm_builtin _ key value left right ->
      case comp key of
        LT -> foldlIf comp f acc left
        EQ ->
          foldlIf comp f (f key value (foldlIf comp f acc left)) right
        GT -> foldlIf comp f acc right
